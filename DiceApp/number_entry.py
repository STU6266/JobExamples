...
class _NumberEntry(Entry):
    ...
    def __init__(self, parent, datatype, dataname,
        ...
        assert type(self) != _NumberEntry, \
            ...
            "only child classes of _NumberEntry can be instantiated"
    ...
    def clear(self):
        """Clear the entry and reset validation style."""
        self.config({"validate": "focusin"})
    ...
    def _validate_key(self, current_text, text_if_allowed):
        ...
        try:
            if not _NumberEntry._contains_space(text_if_allowed):
                ...
                # If text_if_allowed is allowed, we allow it and also check it for validity
                if allowed:
                    valid = self._in_bounds(n)
        except ValueError:
            allowed = (len(text_if_allowed) == 0 or
                       (self.__allow_negative and text_if_allowed == "-"))
        # If not allowed, check the current text (before this keystroke) for validity
        if not allowed:
            try:
                n = int(current_text)
                valid = self._in_bounds(n)
    ...
    def _validate_key(self, current_text, text_if_allowed):
        ...
        try:
            ...
            allowed = (len(text_if_allowed) == 0 or
                       ...
                       (self.__allow_negative and self.__allow_leading_dot and text_if_allowed == "-."))
        # If not allowed, check the current text (before this keystroke) for validity
        if not allowed:
            try:
